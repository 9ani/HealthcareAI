...
class MenuService {
  ...
  async recommendFood(
    ...
    try {
      ...
      const menuData =
        cityName &&
        ["Алматы", "Астана", "Шымкент", "Уральск"].includes(cityName) &&
        places.length > 0
  ...
  private async saveNewMenuItems(cityName: string) {
    if (this.newMenuItems.length > 0) {
      try {
        ...
        if (!menuFileName) {
          console.warn(
            `No menu file found for city: ${cityName}. New items will not be saved.`
          );
          return;
        }
        ...
        console.log(
          `Added ${this.newMenuItems.length} new items to ${filePath}`
        );
    ...
    try {
      ...
      if (menuFileName) {
        try {
          const fileContent = await fs.readFile(
            `menuList/${menuFileName}`,
            "utf-8"
          );
          menuList = JSON.parse(fileContent);
        } catch (readError) {
          console.warn(
            `Error reading ${menuFileName}. Starting with empty menu list.`
          );
        }
      }
      ...
      for (const place of places) {
        ...
        if (menuItem) {
          ...
          console.log(
            `Cafe "${place.name}" not found in ${
              menuFileName || "any menu file"
            }. Parsing menu...`
          );
          const parsedMenu = await this.parseMenu(place.id);
          const newMenuItem = {
...
const scrapeMenuImages = async (placeId: string): Promise<string[]> => {
  const url = `https://2gis.kz/firm/${placeId}/tab/menu`;
